...
app.get("/sitemap.xml", async function(req,res) {
    let link = "<url><loc>https://athenacode.cf/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
        link += "\n<url><loc>https://athenacode.cf/bot/"+bot.botID+"</loc></url>";
    })
  ...
  const renderTemplate = (res, req, template, data = {}) => {
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://athenacode.cf/user/${a.id}) isimli kullanıcı **siteye** giriş yapmaya çalıştı fakat siteden engellendiği için giriş yapamadı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "Athena Code'den engellendiniz.", logout: true })
        req.logout();
        });
        } else {
            try {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://athenacode.cf/user/${a.id}) isimli kullanıcı **siteye** giriş yaptı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
...
//------------------- EXTRA -------------------//
    ...
    app.get("/dc", (req, res) => {
      res.redirect('https://discord.gg/BAJDfkh2v6');
    });  
    ...
    app.post("/code-request", checkMaintence, checkAuth, async (req,res) => {
       client.users.fetch(req.user.id).then(a => {
         ...
         client.channels.cache.get(channels.request).send(new Discord.MessageEmbed()
                              ...
                              .setDescription(`User **[${a.username}#${a.discriminator}](https:// athenacode.cf/user/${a.id})** requested the code named **${rBody['kodadi']}**.`)
    ...
    app.get("/bot/:botID/delete", async (req, res) => {
      ...
      app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         client.users.fetch(req.params.botID).then(bota => {
              client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> kullanıcısının, **${bota.tag}** adlı botu onaylandı. https://athenacode.cf/bot/${bota.id} `)
...
await appsdata.deleteOne({ botID: req.params.botID })
       ...
       app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`**[${req.user.username}](https://athenacode.cf/user/${req.user.id})** isimli kişi sisteme **${rBody['codename']}** adında bir kod ekledi.`)
          .addField("Kod linki", `https://athenacode.cf/code/${kod}`, true)
       ...
       app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`**[${req.user.username}](https://athenacode.cf/user/${req.user.id})** isimli kişi kod düzenledi. İsim: **${rBody['codename']}**.`)
          .addField("Kod linki", `https://athenacode.cf/code/${kod}`, true)
          .addField("Kod açıklaması", rBody['codedesc'], true)
          .addField("Kod kategorisi", rBody['category'], true)
          )
      ...
      app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`Athena Code has been switched to __maintance__ due to **${req.body.reason}**`).then(a => { 
      ...
      app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        .setAuthor("Athena Code", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<a:online:833375738785824788> Athena Code web tekrardan **aktif**!\n[Buraya tıkla ve siteye gir](https://athenacode.cf)`)
        .setFooter("Athena Code © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => { a.edit(`~~ Athena Code Web __maintance__ moduna geçiş yaptı.: **${bakimdata.reason}** ~~`, bakimsonaerdikardesvcodes) } )
